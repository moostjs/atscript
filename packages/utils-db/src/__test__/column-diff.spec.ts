...
function col(name: string, type = 'TEXT', notnull = false, pk = false, dflt_value?: string): TExistingColumn {
...
describe('computeColumnDiff', () => {
  ...
  it('should detect nullable→non-nullable change', () => {
    const desired = [
      field({ physicalName: 'description', optional: false }),
    ]
  ...
  it('should detect non-nullable→nullable change', () => {
    const desired = [
      field({ physicalName: 'priority', optional: true }),
    ]
