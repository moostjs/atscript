...
describe('RESTRICT delete', () => {
  ...
  beforeAll(async () => {
    ...
    const user = await import('./fixtures/restrict-user.as.js')
    const project = await import('./fixtures/restrict-project.as.js')
    const category = await import('./fixtures/restrict-category.as.js')
    UserType = user.User
  ...
  it('should remap native FK_VIOLATION to CONFLICT on delete', async () => {
    ...
    const space = new DbSpace(() => {
      ...
      const origDeleteOne = adapter.deleteOne.bind(adapter)
      adapter.deleteOne = async (filter) => {
