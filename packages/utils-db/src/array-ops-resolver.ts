...
export function resolveArrayOps(
  ...
  for (const [field, ops] of opsMap) {
    ...
    const arrayType = table.flatMap.get(field) as TAtscriptAnnotatedType<TAtscriptTypeArray> | undefined
    const keyProps = arrayType ? getKeyProps(arrayType) : new Set<string>()
    const uniqueItems = arrayType?.metadata?.get('expect.array.uniqueItems') as { message?: string } | undefined
    ...
    const effectiveKeys = ops.keys && ops.keys.length > 0
      ? new Set(ops.keys)
      : keyProps
...
function applyUpdate(arr: unknown[], items: unknown[], keyProps: Set<string>): unknown[] {
  ...
  return arr.map(el => {
    const elObj = el as Record<string, unknown>
    const match = items.find(item => matchByKeys(elObj, item as Record<string, unknown>, keyProps))
    if (match) {
...
function applyUpsert(arr: unknown[], items: unknown[], keyProps: Set<string>, mergeStrategy: boolean): unknown[] {
  const result = [...arr]
  for (const item of items) {
    if (keyProps.size === 0) {
      ...
      const idx = result.findIndex(el => matchByKeys(el as Record<string, unknown>, itemObj, keyProps))
      if (idx >= 0) {
...
function applyInsert(arr: unknown[], items: unknown[], keyProps: Set<string>, uniqueItems: boolean): unknown[] {
  if (!uniqueItems) {
    return [...arr, ...items]
  }
  ...
  const result = [...arr]
  for (const item of items) {
    const exists = keyProps.size > 0
      ? result.some(el => matchByKeys(el as Record<string, unknown>, item as Record<string, unknown>, keyProps))
...
function matchByKeys(a: Record<string, unknown>, b: Record<string, unknown>, keys: Set<string>): boolean {
