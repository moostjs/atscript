...
export class TypeRenderer extends BaseRenderer {
  pre() {
    ...
    this.writeln('/**')
    this.writeln(` * 🪄 This file was generated by Anscript`)
    this.writeln(' * Do not edit this file!')
    this.writeln(' */')
    this.writeln()
    this.writeln('import type { TAnscriptTypeObject } from "@anscript/typescript"')
  }
  ...
  renderStructure(struct: SemanticStructureNode, asClass = false) {
    this.blockln('{}')
    ...
    if (asClass) {
      ...
      this.writeln('static type: TAnscriptTypeObject')
      this.writeln('static metadata: Map<string, unknown>')
    }
    this.pop()
  }
  ...
  renderInterface(node: SemanticInterfaceNode) {
    ...
    if (struct?.entity === 'structure') {
      this.renderStructure(struct as SemanticStructureNode, true)
    } else {
      this.writeln('{}')
    }
    this.writeln()
