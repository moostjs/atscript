...
$('union', TO)
  .item(
    $('object')
      ...
      .prop('age', $().designType('number').tags('number').$type).$type
  )
  .item(
    $('object').prop(
      'kind',
      $('union')
        .item($().designType('string').value('abc').$type)
        .item($().designType('string').value('def').$type).$type
    ).$type
  )
...
$('union', TO2)
  .item(
    $('object')
      .prop('name', $().designType('string').tags('string').$type)
      .prop('age', $().designType('number').tags('number').annotate('meta.label', 'Age').$type)
      .$type
  )
  .item(
    $('object').prop(
      ...
      $('union')
        .item($().designType('string').value('abc').$type)
        .item($().designType('string').value('def').$type)
        .annotate('meta.label', 'Kind').$type
    ).$type
  )
