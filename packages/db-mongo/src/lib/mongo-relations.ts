...
export async function loadRelationsImpl(
  ...
  for (const withRel of withRelations) {
    ...
    relMeta.push({ name: withRel.name, isArray: lookupResult.isArray, relation, nestedWith: extractNestedWith(withRel), stages: lookupResult.stages })
  }
  ...
  if (pkMatchFilter) {
    ...
    const results = await host.collection
      .aggregate(pipeline, host._getSessionOpts())
      .toArray()
