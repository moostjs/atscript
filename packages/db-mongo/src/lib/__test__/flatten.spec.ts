...
describe('asCollection flatten', () => {
  ...
  it('must flatten correctly', async () => {
    ...
    // @ts-expect-error
    expect(table.flatMap.get('complexArray.field1')?.type?.items[0]?.type?.designType).toBe('string')
    // @ts-expect-error
    expect(table.flatMap.get('complexArray.field1')?.type?.items[1]?.type?.designType).toBe('number')
  })
})
