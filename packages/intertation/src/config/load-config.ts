...
import type { TINtertationConfig } from './types'
...
async function bundleTsConfig(configFile: string): Promise<string> {
  ...
  const result = await bundle.write({
    ...
    format: 'esm',
    sourcemap: 'inline',
    entryFileNames: 'intertation.config.[hash].js',
  })
...
const DEFAULT_CONFIG_BASE = 'intertation.config'
...
export async function loadTsConfig(configFile: string): Promise<TINtertationConfig> {
...
export async function loadConfig(configPath: string): Promise<TINtertationConfig> {
