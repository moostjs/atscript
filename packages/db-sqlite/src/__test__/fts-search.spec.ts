...
describe('SqliteAdapter FTS5 Search', () => {
  ...
  beforeAll(async () => {
    ...
    const fixtures = await import('./fixtures/fts-article.as.js')
  ...
  async function seedArticles() {
    await table.insertOne({ title: 'Introduction to Machine Learning', body: 'Machine learning is a subset of artificial intelligence.', category: 'tech' } as any)
    await table.insertOne({ title: 'Database Design Patterns', body: 'Learn about database normalization and indexing strategies.', category: 'tech' } as any)
    await table.insertOne({ title: 'Cooking with Herbs', body: 'Fresh herbs can transform any dish into a culinary masterpiece.', category: 'food' } as any)
    await table.insertOne({ title: 'Advanced Machine Learning', body: 'Deep learning and neural networks push the boundaries of AI.', category: 'tech' } as any)
  }
  ...
  describe('syncIndexes (FTS5)', () => {
    ...
    it('should be idempotent', async () => {
      ...
      const ftsTables = driver.all<{ name: string; sql: string }>(
  ...
  describe('search', () => {
    ...
    it('should combine search with filter', async () => {
      const results = await table.search('database', { filter: { category: 'tech' }, controls: {} } as any)
      expect(results.length).toBeGreaterThan(0)
    ...
    it('should return empty array for no matches', async () => {
      const results = await table.search('xyznonexistentterm123', { filter: {}, controls: {} } as any)
  ...
  describe('searchWithCount', () => {
    ...
    it('should return correct count ignoring limit', async () => {
      const result = await table.searchWithCount('machine', { filter: {}, controls: { $limit: 1 } } as any)
  ...
  describe('trigger sync', () => {
    it('should index rows inserted after FTS creation', async () => {
      await table.insertOne({ title: 'Late Article', body: 'This was added after FTS setup.', category: 'misc' } as any)
      const results = await table.search('Late Article', { filter: {}, controls: {} } as any)
      expect(results).toHaveLength(1)
    })

    it('should remove deleted rows from FTS index', async () => {
      await table.insertOne({ title: 'Temporary Article', body: 'Will be deleted soon.', category: 'misc' } as any)
      // Verify it's found
    ...
    it('should update FTS index on row update', async () => {
      await table.insertOne({ title: 'Unique Zebra', body: 'Unique Zebra body.', category: 'misc' } as any)
      let results = await table.search('Zebra', { filter: {}, controls: {} } as any)
      expect(results).toHaveLength(1)

      // Update both indexed fields so "Zebra" is fully removed
      await table.updateMany({ title: 'Unique Zebra' } as any, { title: 'Changed Giraffe', body: 'Changed Giraffe body.' } as any)
