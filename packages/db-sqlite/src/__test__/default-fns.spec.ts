...
describe('@db.default function defaults', () => {
  ...
  beforeAll(async () => {
    ...
    const fixtures = await import('./fixtures/default-fns.as.js')
  ...
  describe('@db.default.uuid', () => {
    ...
    it('should honor explicit id values', async () => {
      ...
      const result = await table.insertOne({ id: explicitId, label: 'explicit' } as any)
  ...
  describe('@db.default.increment with start value', () => {
    ...
    it('should honor explicit id values', async () => {
      const result = await table.insertOne({ id: 42, label: 'explicit' } as any)

      const row = driver.get<Record<string, unknown>>(
        `SELECT * FROM counters WHERE id = 42`
      )
      expect(row).toBeDefined()
      expect(row?.label).toBe('explicit')
    })
