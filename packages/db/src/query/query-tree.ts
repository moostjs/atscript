import type { TAtscriptAnnotatedType, AtscriptQueryNode, AtscriptQueryFieldRef } from '@atscript/typescript/utils'
import type { FilterExpr } from '@uniqu/core'

export type { AtscriptQueryNode, AtscriptQueryFieldRef, AtscriptQueryComparison, AtscriptRef } from '@atscript/typescript/utils'
...
export function translateQueryTree(
  ...
  if ('$and' in node) {
    return { $and: (node as { $and: AtscriptQueryNode[] }).$and.map(n => translateQueryTree(n, resolveField)) } as FilterExpr
  }
  if ('$or' in node) {
    return { $or: (node as { $or: AtscriptQueryNode[] }).$or.map(n => translateQueryTree(n, resolveField)) } as FilterExpr
  }
  if ('$not' in node) {
    return { $not: translateQueryTree((node as { $not: AtscriptQueryNode }).$not, resolveField) } as FilterExpr
  }
