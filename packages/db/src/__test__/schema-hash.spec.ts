...
function mockReadable(overrides: Partial<{
  ...
  return {
    ...
    fieldDescriptors: overrides.fieldDescriptors ?? [
      { path: 'id', physicalName: 'id', designType: 'number', optional: false, isPrimaryKey: true, ignored: false, storage: 'column' },
      { path: 'name', physicalName: 'name', designType: 'string', optional: false, isPrimaryKey: false, ignored: false, storage: 'column' },
    ],
...
describe('schema-hash', () => {
  describe('computeTableSnapshot', () => {
    ...
    it('should exclude ignored fields', () => {
      const readable = mockReadable({
        fieldDescriptors: [
          { path: 'id', physicalName: 'id', designType: 'number', optional: false, isPrimaryKey: true, ignored: false, storage: 'column' },
          { path: 'temp', physicalName: 'temp', designType: 'string', optional: true, isPrimaryKey: false, ignored: true, storage: 'column' },
        ],
      })
      const snapshot = computeTableSnapshot(readable)
    ...
    it('should include indexes sorted by key', () => {
      const indexes = new Map([
        ['atscript__plain__name', { key: 'atscript__plain__name', type: 'plain', fields: [{ name: 'name', sort: 'asc' }] }],
        ['atscript__unique__email', { key: 'atscript__unique__email', type: 'unique', fields: [{ name: 'email', sort: 'asc' }] }],
      ])
  ...
  describe('computeSchemaHash', () => {
    ...
    it('should change when an index is added', () => {
      const base: TTableSnapshot = {
        tableName: 'users',
        fields: [{ physicalName: 'id', designType: 'number', optional: false, isPrimaryKey: true, storage: 'column' }],
        indexes: [],
        foreignKeys: [],
      }
      const withIndex: TTableSnapshot = {
        ...base,
        indexes: [{ key: 'atscript__unique__email', type: 'unique', fields: [{ name: 'email', sort: 'asc' }] }],
      }
      expect(computeSchemaHash([base])).not.toBe(computeSchemaHash([withIndex]))
    })

    it('should be stable regardless of table order', () => {
      const table1: TTableSnapshot = { tableName: 'a_users', fields: [], indexes: [], foreignKeys: [] }
      const table2: TTableSnapshot = { tableName: 'b_posts', fields: [], indexes: [], foreignKeys: [] }
