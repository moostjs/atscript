...
export class PostgresAdapter extends BaseDbAdapter {
  ...
  private static readonly NATIVE_DEFAULT_FNS: ReadonlySet<TDbDefaultFn> = new Set(['now', 'uuid', 'increment'])
  ...
  override onBeforeFlatten(_type: TAtscriptAnnotatedType): void {
  ...
  override getDesiredTableOptions() { return [] }
  override async getExistingTableOptions() { return [] }
  ...
  private _mapFkError(detail: string, constraint?: string): Array<{ path: string; message: string }> {
  ...
  async insertMany(
    ...
    return this.withTransaction(async () => {
      ...
      const returningSuffix = pkCols.length > 0
        ? ` RETURNING ${pkCols.map(pk => qi(pk)).join(', ')}`
        : ''
  ...
  async count(query: DbQuery): Promise<number> {
    const where = buildWhere(query.filter)
    const tableName = this.resolveTableName()
    const raw = { sql: `SELECT COUNT(*) as cnt FROM ${quoteTableName(tableName)} WHERE ${where.sql}`, params: where.params }
  ...
  async syncColumns(diff: TColumnDiff): Promise<TSyncColumnResult> {
    ...
    for (const { field } of diff.defaultChanged ?? []) {
      ...
      if (field.defaultValue?.kind === 'value') {
        ...
        const fnExpr = field.defaultValue.fn === 'now' ? '(extract(epoch from now()) * 1000)::bigint'
          : field.defaultValue.fn === 'uuid' ? 'gen_random_uuid()'
  ...
  async recreateTable(): Promise<void> {
    ...
    try {
      ...
      const fkRefs = await conn.all<{
        constraint_name: string; table_name: string; table_schema: string
        column_name: string; ref_column_name: string
      ...
      const fkByName = new Map<string, { schema: string; table: string; cols: string[]; refCols: string[]; onDelete: string; onUpdate: string }>()
      for (const fk of fkRefs) {
        let entry = fkByName.get(fk.constraint_name)
        if (!entry) {
          entry = { schema: fk.table_schema, table: fk.table_name, cols: [], refCols: [], onDelete: fk.delete_rule, onUpdate: fk.update_rule }
  ...
  override async searchWithCount(
    ...
    const countPromise = (async () => {
      const raw = { sql: `SELECT COUNT(*) as cnt FROM ${quoteTableName(tableName)} WHERE ${combinedWhere.sql}`, params: combinedWhere.params }
      const { sql, params } = finalizeParams(pgDialect, raw)
      this._log(sql, params)
      const row = await this._exec().get<{ cnt: number | string }>(sql, params)
      return parseCount(row?.cnt)
  ...
  private _prepareVectorSearch(vector: number[], query: DbQuery, indexName?: string) {
    ...
    return { field, vec, distanceOp, where, controls, threshold, tableName: this.resolveTableName(), vectorStr: vectorToString(vector) }
  }
...
function normalizePgType(
  ...
  switch (dt) {
    ...
    case 'numeric': return numericPrecision != null && numericScale != null
...
function normalizePgDefault(value: string | null, isIdentity: string): string | undefined {
  ...
  if (lower === 'current_timestamp' || lower === 'now()' || lower.startsWith('current_timestamp::') || lower.startsWith('now()::')) {
    return 'fn:now'
  }
...
function thresholdToDistance(threshold: number, similarity: string): number {
  switch (similarity) {
    case 'euclidean': return threshold     // user provides max distance directly
    case 'dotProduct': return -threshold   // pgvector uses negative inner product
    default: return 2 * (1 - threshold)    // cosine: score → pgvector distance
  }
}
