...
import {
  qi, quoteTableName, pgTypeFromField, buildCreateTable, collationToPg, pgDialect, offsetPlaceholders,
  defaultValueForType, defaultValueToSqlLiteral,
} from '../sql-builder'
...
describe('pgTypeFromField', () => {
  ...
  it('maps string PK to VARCHAR(255)', () => {
    expect(pgTypeFromField(field({ designType: 'string', isPrimaryKey: true }))).toBe('VARCHAR(255)')
  })
  ...
  it('maps signed int types to matching PG types', () => {
    const mkField = (tag: string) => field({
      designType: 'number',
...
describe('buildCreateTable', () => {
  it('uses GENERATED BY DEFAULT AS IDENTITY for increment', () => {
    const fields: TDbFieldMeta[] = [
      {
        path: 'id', physicalName: 'id', designType: 'number',
        optional: false, isPrimaryKey: true, ignored: false,
        defaultValue: { kind: 'fn', fn: 'increment' },
      } as TDbFieldMeta,
    ]
  ...
  it('uses gen_random_uuid() for uuid default', () => {
    const fields: TDbFieldMeta[] = [
      {
        path: 'id', physicalName: 'id', designType: 'string',
        optional: false, isPrimaryKey: true, ignored: false,
  ...
  it('uses epoch ms expression for now default (BIGINT)', () => {
    const fields: TDbFieldMeta[] = [
      {
        path: 'createdAt', physicalName: 'created_at', designType: 'number',
        optional: true, isPrimaryKey: false, ignored: false,
        defaultValue: { kind: 'fn', fn: 'now' },
      } as TDbFieldMeta,
    ]
  ...
  it('uses false/true for boolean defaults (not 0/1)', () => {
    const fields: TDbFieldMeta[] = [
      ...
      {
        path: 'completed', physicalName: 'completed', designType: 'boolean',
        optional: false, isPrimaryKey: false, ignored: false,
