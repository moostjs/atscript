...
import type { AtscriptDbTableLike, AtscriptDbWritable, TDbForeignKey, TDbRelation, TWriteTableResolver } from '../types'
import type { TableMetadata } from './table-metadata'
import { resolveRelationTargetTable } from './relation-loader'
import { wrapNestedError } from './error-utils'
...
export interface TNestedWriterHost {
  ...
  findOne(query: { filter: FilterExpr; controls: Record<string, never> }): Promise<Record<string, unknown> | null>
}
  ...
  for (const [navField, relation] of host._meta.relations) {
    ...
    for (const item of items) {
      ...
      if (fkValue === undefined) {
        ...
        const current = await host.findOne({ filter: pkFilter, controls: {} as Record<string, never> })
        if (!current) {
...
export async function batchPatchNestedFrom(
  ...
  for (const [navField, relation] of host._meta.relations) {
    ...
    for (const original of originals) {
      ...
      // $replace
      if (ops.replace) {
        await fromReplace(targetTable, ops.replace, parentPK, fkField, childPKs, navField, maxDepth, depth)
      }
...
export async function batchPatchNestedVia(
  ...
  for (const [navField, relation] of host._meta.relations) {
    ...
    for (const original of originals) {
      ...
      // $replace
      if (ops.replace) {
        await viaReplace(targetTable, junctionTable, ops.replace, parentPK, targetPKField, fkToThis.fields[0], fkToTarget.fields[0], maxDepth, depth)
      }
