...
// ── Exported helpers (used by db-table.ts write path) ────────────────────────
...
async function loadToRelation(
  ...
  if (localFields.length === 1) {
    ...
    const related = await queryCompositeFK(rows, { localFields, targetFields, targetTable, relQuery })
...
async function loadFromRelation(
  ...
  if (localFields.length === 1) {
    ...
    const related = await queryCompositeFK(rows, { localFields, targetFields: remoteFields, targetTable, relQuery })
...
async function loadViaRelation(
  ...
  if (localPKFields.length === 1) {
    await loadViaSingleKey(rows, {
      relName, relation, targetTable, relQuery,
      localPKFields, junctionLocalFields, targetPKFields, junctionTargetFields,
      junctionTable,
    })
  } else {
...
function compositeKey(fields: string[], obj: Record<string, unknown>): string {
  ...
  for (let i = 0; i < fields.length; i++) {
    ...
    key += v === null || v === undefined ? '\0' : String(v) // null or undefined becomes empty string, distinct from literal '\0' value.
  }
  return key
}
