...
export class SyncStore {
  ...
  async readTableSnapshot(tableName: string, _asView?: boolean): Promise<TTableSnapshot | TViewSnapshot | null> {
    const value = await this.readControlValue(`table_snapshot:${tableName}`)
    return value ? JSON.parse(value) : null
  }

  async writeTableSnapshot(tableName: string, snapshot: TTableSnapshot | TViewSnapshot): Promise<void> {
  ...
  async readTrackedList(): Promise<Array<{ name: string; isView: boolean; viewType?: 'V' | 'M' | 'E' }>> {
