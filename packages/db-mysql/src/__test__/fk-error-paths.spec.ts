...
import { AtscriptDbTable, DbError } from '@atscript/utils-db'
...
describe('MysqlAdapter — FK error mapping', () => {
  beforeAll(async () => {
    ...
    const fixtures = await import('./fixtures/fk-tables.as.js')
  ...
  it('should extract FK column from error message and map to field path', async () => {
    const err = Object.assign(new Error('FK violation'), {
      errno: 1452,
      sqlMessage: 'Cannot add or update a child row: a foreign key constraint fails (`db`.`tasks`, CONSTRAINT `tasks_ibfk_1` FOREIGN KEY (`projectId`) REFERENCES `projects` (`id`))',
      message: 'Cannot add or update a child row: a foreign key constraint fails (`db`.`tasks`, CONSTRAINT `tasks_ibfk_1` FOREIGN KEY (`projectId`) REFERENCES `projects` (`id`))',
    })
  ...
  it('should remap FK error on delete (errno 1451) to CONFLICT via _remapDeleteFkViolation', async () => {
    ...
    const err = Object.assign(new Error('FK violation'), {
      errno: 1451,
      sqlMessage: 'Cannot delete or update a parent row: a foreign key constraint fails (`db`.`tasks`, CONSTRAINT `tasks_ibfk_1` FOREIGN KEY (`projectId`) REFERENCES `projects` (`id`))',
      message: 'Cannot delete or update a parent row: a foreign key constraint fails (`db`.`tasks`, CONSTRAINT `tasks_ibfk_1` FOREIGN KEY (`projectId`) REFERENCES `projects` (`id`))',
    })
  ...
  it('should pass through non-constraint errors unchanged', async () => {
    ...
    await expect(
      table.insertOne({ id: 1, title: 'Test', projectId: 1 } as any)
