...
describe('MysqlAdapter + AtscriptDbTable', () => {
  ...
  describe('ensureTable', () => {
    it('should emit CREATE TABLE IF NOT EXISTS', async () => {
      ...
      const execCall = driver.calls.find(c => c.method === 'exec' && c.sql.includes('CREATE TABLE'))
  ...
  describe('insertOne', () => {
    it('should emit INSERT INTO with backtick-quoted columns', async () => {
      await table.insertOne({ id: 1, email: 'test@x.com', name: 'Test', createdAt: 1000, status: 'active' } as any)
    ...
    it('should return insertedId from data when @meta.id is present', async () => {
      const result = await table.insertOne({ id: 42, email: 'test@x.com', name: 'Test', createdAt: 1000, status: 'active' } as any)
    ...
    it('should fall back to db-generated insertId when no PK in data', async () => {
      ...
      const result = await t.insertOne({ email: 'x@x.com', name: 'X', createdAt: 1000, status: 'ok' } as any)
  ...
  describe('_wrapConstraintError', () => {
    ...
    it('should rethrow non-MySQL errors as-is', async () => {
      ...
      await expect(t.insertOne({ id: 1, email: 'x', name: 'X', createdAt: 1, status: 'a' } as any))
  ...
  describe('transactions', () => {
    ...
    it('should release connection after ROLLBACK', async () => {
      ...
      try {
        await adapter.withTransaction(async () => { throw new Error('Fail') })
      } catch {
        // expected
      }
      expect(conn!.release).toHaveBeenCalled()
