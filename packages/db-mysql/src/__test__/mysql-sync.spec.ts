...
import type { TColumnDiff, TDbFieldMeta } from '@atscript/utils-db'
...
describe('MysqlAdapter — schema sync', () => {
  ...
  describe('syncColumns', () => {
    it('should emit ALTER TABLE ADD COLUMN for new fields', async () => {
      ...
      const diff: TColumnDiff = {
        added: [
          { physicalName: 'age', designType: 'integer', optional: false, isPrimaryKey: false } as TDbFieldMeta,
        ],
        renamed: [],
        typeChanged: [],
        nullableChanged: [],
    ...
    it('should emit ALTER TABLE RENAME COLUMN for renamed fields', async () => {
      ...
      const diff: TColumnDiff = {
        added: [],
        renamed: [
          { field: { physicalName: 'full_name', designType: 'string', optional: false } as TDbFieldMeta, oldName: 'name' },
        ],
        typeChanged: [],
        nullableChanged: [],
        dropped: [],
    ...
    it('should emit ALTER TABLE MODIFY COLUMN for type changes', async () => {
      ...
      const diff: TColumnDiff = {
        ...
        renamed: [],
        typeChanged: [
          { field: { physicalName: 'score', designType: 'number', optional: false, isPrimaryKey: false } as TDbFieldMeta, oldType: 'INT' },
        ],
        nullableChanged: [],
        dropped: [],
      }
  ...
  describe('recreateTable', () => {
    ...
    it('should create temp table, copy data, drop old, rename', async () => {
      ...
      expect(sqls.some(s => s.includes('CREATE TABLE IF NOT EXISTS') && s.includes('__tmp_'))).toBe(true)
      // Should copy data
  ...
  describe('syncIndexes', () => {
    ...
    it('should emit CREATE INDEX for missing indexes', async () => {
      ...
      await table.syncIndexes()
      const createCalls = driver.calls.filter(c => c.sql.includes('CREATE') && c.sql.includes('INDEX'))
  ...
  describe('syncForeignKeys', () => {
    ...
    beforeAll(async () => {
      FkFixtures = await import('./fixtures/fk-tables.as.js')
    })

    it('should query INFORMATION_SCHEMA.KEY_COLUMN_USAGE for existing FKs', async () => {
      const driver = createSyncMockDriver()
      const adapter = new MysqlAdapter(driver)
      const table = new AtscriptDbTable(FkFixtures.Task, adapter)
    ...
    it('should emit ALTER TABLE DROP FOREIGN KEY for stale FKs', async () => {
      const existingFks = [
        { CONSTRAINT_NAME: 'fk_old', COLUMN_NAME: 'old_col', REFERENCED_TABLE_NAME: 'other', REFERENCED_COLUMN_NAME: 'id' },
      ]
      const driver = createSyncMockDriver({
