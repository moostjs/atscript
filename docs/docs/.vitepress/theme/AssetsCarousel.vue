...
const cards = [
  ...
  {
    href: 'https://db.atscript.dev/guide/',
  ...
  {
    href: 'https://db.atscript.dev/http/crud',
    title: 'REST API',
    status: 'Available today',
    tone: 'today',
...
function getPosition(index) {
  ...
  if (diff === 1 || (diff === count - 1 && count <= 2)) {
    return 'right'
  }
  if (diff === count - 1) {
    return 'left'
  }
  if (diff === 2 || (diff === count - 2 && count <= 3)) {
    return 'far-right'
  }
  if (diff === count - 2) {
    return 'far-left'
      ...
      <component
        ...
        @click="
          card.position === 'center'
            ? undefined
            : card.position === 'left'
              ? (prev(), stopPermanently())
              : card.position === 'right'
                ? (next(), stopPermanently())
                : undefined
        "
      >
