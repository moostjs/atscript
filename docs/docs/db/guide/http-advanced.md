...
Load related data inline using `$with`. Relations must be declared with `@db.rel.to`, `@db.rel.from`, or `@db.rel.via` in your `.as` schema. See [Relations — Foreign Keys](/db/relations/) for how to define them.
...
[
  {
    ...
    "author": { "id": 5, "name": "Alice" },
    "comments": [
      { "id": 10, "text": "Great task!", "todoId": 1 }
    ]
  }
]
```
...
For the programmatic equivalent, see [Relations — Loading](/db/relations/loading).
...
Full-text search support varies by adapter. MongoDB supports Atlas Search with named indexes. SQLite provides FTS5-based search. Check your [adapter's documentation](/db/adapters/) for details.
:::

For annotations and programmatic API, see [Text Search](/db/search/).
...
```json
[
  { "region": "US", "total": 15420.50, "orders": 87 },
  { "region": "EU", "total": 8930.00, "orders": 42 }
]
```
...
The standard SQL aggregate functions are available: `count(*)`, `sum(field)`, `avg(field)`, `min(field)`, `max(field)`. See [Aggregation Annotations](/db/views/aggregations) for the full reference.
...
`POST`, `PUT`, and `PATCH` request bodies can contain nested relation data. The controller processes nested objects through the [deep operations](/db/relations/deep-operations) pipeline.
...
For full details on how nested data is processed across foreign keys, see [Relations — Deep Operations](/db/relations/deep-operations).
