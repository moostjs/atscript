...
- **[Relations](/db/relations/)** — TO (foreign key), FROM (reverse 1:N), and VIA (M:N junction table) with explicit `$with` loading
- **[Views](/db/views/)** — managed, materialized, and external views defined with `@db.view` annotations
- **[Text search](/db/search/)** — full-text search across indexed fields
