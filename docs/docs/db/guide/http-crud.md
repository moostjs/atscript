...
See [HTTP Setup](./http-setup) for installation and wiring.
...
See [URL Query Syntax](./http-query-syntax) for the full filter syntax and [Relations & Search](./http-advanced) for `$with`, `$search`, `$vector`, and `$groupBy`.
...
Default values from `@db.default` and generated defaults (`@db.default.increment`, `@db.default.uuid`, `@db.default.now`) are applied automatically. Request bodies can contain nested relation data for [deep insert operations](/db/relations/deep-operations).
...
Nested relation data is supported per item — each record goes through the [deep replace](/db/relations/deep-operations) process.
...
See [Update & Patch](/db/guide/update-patch) for details on patch semantics and embedded object updates.
...
- [URL Query Syntax](./http-query-syntax) — Full filter, sort, and pagination syntax
- [Relations & Search in URLs](./http-advanced) — `$with`, `$search`, `$vector`, `$groupBy` in query strings
- [Customization](./http-customization) — Override hooks for access control and data transformation
- [CRUD Operations](/db/guide/crud) — Programmatic `AtscriptDbTable` API (non-HTTP)
