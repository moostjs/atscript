...
Provides only read endpoints — no write operations. Use for [views](/db/views/) or tables where writes aren't needed:
...
See [Setup](/db/guide/setup) for details on configuring `DbSpace`.
