...
::: tip
`@db.index.fulltext` sets up the index in the database. For search usage (querying against full-text indexes), see [Text Search](/db/search/).
:::
...
Each adapter translates these to its native collation. For example, PostgreSQL maps `'nocase'` to the `CITEXT` type, while SQLite uses the `NOCASE` collation. See [Adapters](/db/adapters/) for adapter-specific collation details.
