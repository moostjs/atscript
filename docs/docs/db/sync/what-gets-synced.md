...
For more on foreign key annotations, see [Foreign Keys](/db/relations/).
...
Schema sync manages views according to their type (see [View Types](/db/views/view-types) for definitions):
...
For more on view definitions and types, see [Views](/db/views/).
